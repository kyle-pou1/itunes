    ...
    this.getSongData = function(artist) {
      ...
      $http({
        ...
        .then(function(response){
          var pData = response.data.results;
          var parsedResults = [];
          console.log(response);

          for (var i = 0; i < pData.length; i++) {
            parsedResults.push({
              Play: pData[i].previewUrl,
              Artist: pData[i].artistName,
              Collection: pData[i].collectonCensoredName,
              AlbumArt: pData[i].artworkUrl60,
              Type: pData[i].kind,
              CollectionPrice: pData[i].collectionPrice,
            });
          }
